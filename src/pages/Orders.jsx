    ...
    <div className="rounded-2xl border border-gray-200 p-5 shadow-sm bg-white space-y-4">
      ...
      <div className="mt-2">
        <div className="relative pl-6">
          ...
          <div
            className="absolute left-2 top-2 w-0.5 bg-emerald-600 transition-all duration-700"
            style={{ height: `${prog}%` }}
          ></div>
          {steps.map((s,i)=>(
            <div key={s} className="flex items-center mb-2">
              <div className={`relative w-3 h-3 rounded-full mr-3 ${i<=currentIndex ? "bg-emerald-600" : "bg-gray-300"}`}>
                {i===currentIndex ? <span className="absolute inset-0 rounded-full bg-emerald-400 opacity-60 animate-ping"></span> : null}
              </div>
              <div className={`${i<=currentIndex ? "text-emerald-700 font-medium" : "text-gray-500"}`}>{s}</div>
            </div>
          ))}
        </div>
      </div>
      <div className="text-sm">
        <span className={`px-2 py-1 rounded-full border text-xs ${o.orderStatus==='Delivered'?'border-emerald-300 text-emerald-700 bg-emerald-50':'border-gray-300 text-gray-700 bg-gray-50'}`}>{o.orderStatus}</span>
